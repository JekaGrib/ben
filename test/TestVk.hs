...
handle0,handle2,handle3,handle4,handle5,handle6,handle7 :: Handle (StateT [MockAction] IO)
...
testVk = hspec $ do
  ...
  describe "(getServer >>= runServ)" $ do
    it "work with empty update list" $ do
      actions <- execStateT (evalStateT (getServer handle0 >> runServ handle0) (ServerInfo "A" "A" "1",initialDB1) ) []
    ...
    it "work with singleton update list with text msg" $ do
      actions <- execStateT (evalStateT (getServer handle2 >> runServ handle2) (ServerInfo "A" "A" "1",initialDB1) ) []
      reverse actions `shouldBe`
        [LOGMSG DEBUG "Send request to getLongPollServer: https://api.vk.com/method/groups.getLongPollServer?group_id=321&access_token=ABC123&v=5.103\n",
        GOTSERVER,
    ...
    it "work with singleton update list with sticker msg " $ do
      actions <- execStateT (evalStateT (getServer handle5 >> runServ handle5) (ServerInfo "A" "A" "1",initialDB1) ) []
      reverse actions `shouldBe`
        [LOGMSG DEBUG "Send request to getLongPollServer: https://api.vk.com/method/groups.getLongPollServer?group_id=321&access_token=ABC123&v=5.103\n", 
        GOTSERVER,
        LOGMSG DEBUG $ "Get response: " ++ show json1 ++ "\n",
...
jsonA :: ResponseS
jsonA = "{\"response\":{\"key\":\"912481cc91cb3b0e119b9be5c75b383d6887438f\",\"server\":\"https:\\/\\/lp.vk.com\\/wh000\",\"ts\":\"289\"}}"