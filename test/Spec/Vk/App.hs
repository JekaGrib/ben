...
import Vk.AppT (AppState (..), TryServer (..), firstTry, nextTry, secondTry, thirdTry)
  ...
  hspec $ do
    ...
    describe "runServ" $ do
      it "work with empty update list" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle10) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
      ...
      it "work with singleton update list with \"love\" text msg" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle11) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
                       SENDMSG 123 "love",
      ...
      it "work with /repeat text msg(send Keyb,put user in OpenRepeat mode)" $ do
        (AppState _ mapUN, actions) <-
          runStateT
            (execStateT (runServ handle43) $ AppState emptyTryServInf initialDB3)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
                       SENDKEYB 123 3 " : Current number of repeats your message.\nWhy?"
                     ]
        mapUN `shouldBe` Map.insert 123 (Left (OpenRepeat 3)) initialDB3
      it "work with /help text msg(send infoMsg)" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle44) $ AppState emptyTryServInf initialDB3)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
                       SENDMSG 123 "Hello"
      ...
      it
        "work with text msg \"love\" if user is in OpenRepeat mode\
        \ (send warning info msg)"
        $ do
          actions <-
            execStateT
              (evalStateT (runServ handle11) $ AppState emptyTryServInf initialDB2)
              []
          reverse actions
            `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
                         LOG WARNING,
      ...
      it
        "work with text msg \"3\" if user is in (OpenRepeat 4) mode\
        \ (change N to 3, send info msg)"
        $ do
          (AppState _ mapUN, actions) <-
            runStateT
              (execStateT (runServ handle41) $ AppState emptyTryServInf initialDB2)
              []
          reverse actions
            `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
                         SENDMSG 123 "Number of repeats successfully changed from 4 to 3"
                       ]
          mapUN `shouldBe` Map.insert 123 (Right 3) initialDB2
      it
        "work with text msg \"love\" after text msg \"3\" if user is in (OpenRepeat 4) mode\
        \ (change N to 3, send info msg,send \"love\" 3 times)"
        $ do
          (AppState _ mapUN, actions) <-
            runStateT
              (execStateT (runServ handle42) $ AppState emptyTryServInf initialDB2)
              []
          reverse actions
            `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
                         SENDMSG 123 "Number of repeats successfully changed from 4 to 3",
      ...
      it "work with singleton update list with sticker msg " $ do
        actions <-
          execStateT
            (evalStateT (runServ handle12) $ AppState emptyTryServInf initialDB3)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
      ...
      it "work with singleton update list with photo attachment msg " $ do
        actions <-
          execStateT
            (evalStateT (runServ handle8) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
      ...
      it "work with singleton update list with doc attachment msg " $ do
        actions <-
          execStateT
            (evalStateT (runServ handle9) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
      ...
      it "work with singleton update list with audio message attachment msg" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle13) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
      ...
      it "work with singleton update list with video attachment msg" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle14) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
      ...
      it "work with singleton update list with audio attachment msg" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle15) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
      ...
      it "work with singleton update list with market attachment msg" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle16) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
      ...
      it "work with singleton update list with wall attachment msg" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle17) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
      ...
      it "work with singleton update list with poll attachment msg" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle18) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
      ...
      it "work with singleton update list with audio attachment msg with text" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle19) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
      ...
      it "work with singleton update list with market attachment msg with text" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle20) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
      ...
      it "work with forward msg (IGNORE)" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle21) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
                       LOG WARNING
      ...
      it "work with singleton update list with GEO msg" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle22) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
      ...
      it "work with singleton update list with GEO msg with text" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle23) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
      ...
      it "work with singleton update list with GEO msg with audio attachment" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle24) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
                       SENDAttachMSG
      ...
      it "work with singleton update list with GEO msg with audio attachment with text" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle25) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
                       SENDAttachMSG
      ...
      it "work with singleton update list with GEO msg with photo attachment with text" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle26) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
                       VkMock $ GOTPhotoSERVER 1606,
      ...
      it "work with singleton update list with GEO forward msg (IGNORE)" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle27) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
                       LOG WARNING
      ...
      it "work with update list with several text msgs from different users" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle28) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
                       SENDMSG 123 "love",
      ...
      it "work with update list with several attachment msgs from different users" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle29) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
      ...
      it "work with update list with several attachments in one msg" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle30) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
                       VkMock $ GOTPhotoSERVER 1606,
      ...
      it "work with update list with several attachments with text in one msg" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle31) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
                       VkMock $ GOTPhotoSERVER 1606,
      ...
      it "work with update list with several attachments with text in one GEO msg" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle32) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
                       VkMock $ GOTPhotoSERVER 1606,
      ...
      it "work with update list with Unknown update (IGNORE only unknown update)" $ do
        actions <-
          execStateT
            (evalStateT (runServ handle33) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
                       LOG WARNING,
      ...
      it "work with update list with sticker msg with text (IGNORE) " $ do
        actions <-
          execStateT
            (evalStateT (runServ handle34) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
                       LOG WARNING
      ...
      it "work with update list with sticker and other attachment in one msg (IGNORE) " $ do
        actions <-
          execStateT
            (evalStateT (runServ handle35) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
                       LOG WARNING
      ...
      it "throw CheckGetUpdatesException with unknown getUpdates answer" $
        runStateT (evalStateT (runServ handle36) $ AppState emptyTryServInf initialDB1) []
          `shouldThrow` isCheckGetUpdatesResponseException
      it "throw CheckGetUpdatesException with error getUpdates answer" $
        runStateT (evalStateT (runServ handle6) $ AppState emptyTryServInf initialDB1) []
          `shouldThrow` isCheckGetUpdatesResponseException
      it "got new server info and send nothing if getUpdates answer=fail2 FirstTime" $ do
        newTryServInfo <-
          tryServer
            <$> evalStateT (execStateT (runServ handle37) $ AppState emptyTryServInf initialDB1) []
        newTryServInfo
          `shouldNotBe` emptyTryServInf
        actions <-
          execStateT (evalStateT (runServ handle37) $ AppState emptyTryServInf initialDB1) []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
                       LOG WARNING,
                       VkMock GOTSERVER
      ...
      it "got new server info and send nothing if getUpdates answer=fail2 SecondTime" $ do
        newTryServInfo <-
          tryServer
            <$> evalStateT
              (execStateT (runServ handle37) $ AppState (secondTry emptyServInf) initialDB1)
              []
        newTryServInfo
          `shouldNotBe` emptyTryServInf
        actions <-
          execStateT
            (evalStateT (runServ handle37) $ AppState (secondTry emptyServInf) initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
                       LOG WARNING,
      ...
      it "change FirstTry to SecondTry if getUpdates answer=fail2 FirstTime" $ do
        newTryServInfo <-
          tryServer
            <$> evalStateT
              (execStateT (runServ handle37) $ AppState emptyTryServInf initialDB1)
              []
        tryNum newTryServInfo `shouldBe` 2
      it "throw exception if getUpdates answer=fail2 ThirdTime" $
        evalStateT
          (execStateT (runServ handle37) $ AppState (thirdTry emptyServInf) initialDB1)
          []
          `shouldThrow` isCheckGetUpdatesResponseException
      it "got new server info and send nothing if getUpdates answer=fail3 FirstTime" $ do
        newTryServInfo <-
          tryServer
            <$> evalStateT
              (execStateT (runServ handle38) $ AppState emptyTryServInf initialDB1)
              []
        newTryServInfo
          `shouldNotBe` emptyTryServInf
        actions <-
          execStateT
            (evalStateT (runServ handle38) $ AppState emptyTryServInf initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
                       LOG WARNING,
      ...
      it "got new server info and send nothing if getUpdates answer=fail3 SecondTime" $ do
        newTryServInfo <-
          tryServer
            <$> evalStateT
              (execStateT (runServ handle38) $ AppState (secondTry emptyServInf) initialDB1)
              []
        newTryServInfo
          `shouldNotBe` emptyTryServInf
        actions <-
          execStateT
            (evalStateT (runServ handle38) $ AppState (secondTry emptyServInf) initialDB1)
            []
        reverse actions
          `shouldBe` [ VkMock $ GOTUPDATES emptyServInf,
                       LOG WARNING,
      ...
      it "change FirstTry to SecondTry if getUpdates answer=fail3 FirstTime" $ do
        newTryServInfo <-
          tryServer
            <$> evalStateT (execStateT (runServ handle38) $ AppState emptyTryServInf initialDB1) []
        tryNum newTryServInfo `shouldBe` 2
      it "throw exception if getUpdates answer=fail3 ThirdTime" $
        evalStateT (execStateT (runServ handle38) $ AppState (thirdTry emptyServInf) initialDB1) []
          `shouldThrow` isCheckGetUpdatesResponseException
      it
        "change ts in serverInfo and FirstTry to SecondTry\
        \ if getUpdates answer=FailTs(ts=25) FirstTime"
        $ do
          newTryServInfo <-
            tryServer
              <$> evalStateT
                (execStateT (runServ handle39) $ AppState emptyTryServInf initialDB1)
                []
          newTryServInfo `shouldBe` nextTry (emptyTryServInf {servInf = emptyServInf {tsSI = 25}})
      it
        "change ts in serverInfo and FirstTry to SecondTry\
        \ if getUpdates answer=FailTs(ts=25,fail=1) FirstTime"
        $ do
          newTryServInfo <-
            tryServer
              <$> evalStateT
                (execStateT (runServ handle40) $ AppState emptyTryServInf initialDB1)
                []
          newTryServInfo `shouldBe` nextTry (emptyTryServInf {servInf = emptyServInf {tsSI = 25}})
      it "throw exception if getUpdates answer=failTs(ts=25) ThirdTime" $
        evalStateT
          (execStateT (runServ handle39) $ AppState (thirdTry emptyServInf) initialDB1)
          []
          `shouldThrow` isCheckGetUpdatesResponseException
      it "throw GetUpdatesException if getUpdates HttpException" $
        runStateT (runStateT (runServ handle46) $ AppState emptyTryServInf initialDB1) []
          `shouldThrow` isGetUpdatesException
      it "throw SendMsgException if sendMsg HttpException" $
        runStateT (runStateT (runServ handle47) $ AppState emptyTryServInf initialDB1) []
          `shouldThrow` (isSendMsgException :: Selector (BotException VkAttachMSG))
      it "throw SendKeybException if sendKeyb HttpException" $
        runStateT (runStateT (runServ handle48) $ AppState emptyTryServInf initialDB1) []
      ...
      it "throw GetLongPollServerException if getServer HttpException; getUpdates answer=Fail" $
        runStateT (runStateT (runServ handle49) $ AppState emptyTryServInf initialDB1) []
          `shouldThrow` isGetLongPollServerException
        ...
        actions <-
          execStateT
            ( do
                ...
                evalStateT (runServ handle0) $ AppState tryServ initialDB1
            )
            []
        reverse actions
          `shouldBe` [ LOGMSG
                         ...
                         \group_id=321&access_token=ABC123&v=5.85",
                       VkMock GOTSERVER,
                       ...
                       LOGMSG
                         ...
                         \ https:anyServer?act=a_check&key=anyKey&ts=289&wait=20",
                       VkMock $ GOTUPDATES $
                         ServerInfo
                           "anyKey"
                           "https:anyServer"
        ...
        actions <-
          execStateT
            ( do
                ...
                evalStateT (runServ handle2) $ AppState tryServ initialDB1
            )
            []
        reverse actions
          `shouldBe` [ LOGMSG
                       ...
                       LOGMSG
                         DEBUG
                         "Get response: \"{\\\"response\\\":{\\\"key\\\":\\\"anyKey\\\",\
                         \\\\"server\\\":\\\"https:anyServer\\\",\\\"ts\\\":\\\"289\\\"}}\"",
                       LOGMSG INFO "Work with received server",
                       LOGMSG
                         DEBUG
                         "Send request to getUpdates:\
                       ...
                       LOGMSG
                         DEBUG
                         "Get response: \"{\\\"ts\\\":\\\"290\\\",\\\"updates\\\":[\
                         \{\\\"type\\\":\\\"message_new\\\",\\\"object\\\":\
                         \{\\\"date\\\":1594911394,\\\"from_id\\\":123,\\\"id\\\":597,\
                         \\\\"out\\\":0,\\\"peer_id\\\":1606,\\\"text\\\":\\\"love\\\",\
                         \\\\"conversation_message_id\\\":562,\\\"fwd_messages\\\":[],\
                         \\\\"important\\\":false,\\\"random_id\\\":0,\\\"attachments\\\":[],\
                         \\\\"is_hidden\\\":false},\\\"group_id\\\":19495,\
                         \\\\"event_id\\\":\\\"123\\\"}]}\\r\\n\"",
        ...
        actions <-
          execStateT
            ( do
                ...
                evalStateT (runServ handle5) $ AppState tryServ initialDB1
            )
            []
        reverse actions
          `shouldBe` [ LOGMSG
          ...
          ++ ( concat . replicate 2 $
                 [ LOGMSG
                     DEBUG
                     "Send request to send msg AttachMsg (StickerMsg 9014)\
                     \ to userId 1606",
                   SENDAttachMSG 1606 (StickerMsg 9014),
                   LOGMSG DEBUG "Get response: \"{\\\"response\\\":626}\"",
      ...
      it "throw CheckGetUpdatesException with error getUpdates answer" $
        evalStateT
          ( do
              ...
              evalStateT (runServ handle6) $ AppState tryServ initialDB1
          )
          []
          `shouldThrow` ( ==
      ...
      it "throw CheckGetUpdatesException with unknown getUpdates answer" $
        evalStateT
          ( do
              ...
              evalStateT (runServ handle7) $ AppState tryServ initialDB1
          )
          []
          `shouldThrow` ( ==
                            ( CheckGetUpdatesResponseException $
    ...
    describe "run" $ do
      it
        "throw CheckGetUpdatesResponseException if getUpdates forever answer=fail2\
        \ (more then 2 times) "
        $ evalStateT (run handle37 initialDB1) []
          `shouldThrow` isCheckGetUpdatesResponseException
      it
        "throw CheckGetUpdatesResponseException if getUpdates forever answer=fail3\
        \ (more then 2 times) "
        $ evalStateT (run handle38 initialDB1) []
          `shouldThrow` isCheckGetUpdatesResponseException
      it
        "throw CheckGetUpdatesResponseException if getUpdates forever answer=FailTs(ts=25)\
        \ (more then 2 times) "
        $ evalStateT (run handle39 initialDB1) []
          `shouldThrow` isCheckGetUpdatesResponseException
      it
        "throw CheckGetUpdatesResponseException if getUpdates \
        \forever answer=FailTs(ts=25,fail=1) (more then 2 times) "
